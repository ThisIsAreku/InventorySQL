...
public class Main extends JavaPlugin {
	...
	public static String[] MYSQL_FIELDS = new String[] { "id", "owner",
	...
	public void onEnable() {
		...
		try {
			MYSQLDB = new Database("jdbc:mysql://" + this.dbHost + ":3306/"
		...
		this.getServer()
				.getScheduler()
				.scheduleAsyncRepeatingTask(this, new UpdateDatabase(this),
		...
		try {
			...
			if (!this.MYSQLDB.tableExist(this.dbTable)) {
				...
				if (!this.MYSQLDB.queryBool(query)) {
					log(Level.SEVERE,
							"Cannot create table, check your config !");
				}
			} else {
