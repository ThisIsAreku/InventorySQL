...
ALTER IGNORE TABLE `%%TABLENAME%%` ADD `ischest` TINYINT( 1 ) NOT NULL DEFAULT '0' AFTER `world`;
...
ALTER IGNORE TABLE `%%TABLENAME%%` ADD `last_update` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER `pendings`;
...
ALTER IGNORE TABLE `%%TABLENAME%%` CHANGE `last_update` `last_update` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP;
ALTER IGNORE TABLE `%%TABLENAME%%` CHANGE `owner` `owner` VARCHAR( 32 ) CHARACTER SET utf8 COLLATE utf8_general_ci NULL;